...
function validate(manifest, file) {
  ...
  assert(typeof manifest.latest?.image === 'string' && manifest.latest.image.startsWith('ghcr.io/javdbviewed/'), `${file}: latest.image must be a JavdBviewed GHCR image`)
  assert(typeof manifest.latest?.releaseNotesUrl === 'string' && manifest.latest.releaseNotesUrl.startsWith('https://github.com/JavdBviewed/JavdBviewed-release/'), `${file}: latest.releaseNotesUrl must be hosted by this repository`)
